...
public class RegionFinderExtra {
	...
	private ArrayList<ArrayList<ArrayList<Point>>> colorRegions; //list of differently colored regions
	private int size;										//helper variable for largestRegion method
															// so the identified regions are in a list of lists of points

	public RegionFinderExtra() {
		this.image = null;
		size = 0;
		colorRegions = new ArrayList<ArrayList<ArrayList<Point>>>(); //initialized in constructor to prevent data erasure when finder is called twice
	 ...
	 */
	public ArrayList<ArrayList<ArrayList<Point>>> getColorRegions()
	{
		return colorRegions;
	}
	...
	/**
	 * clears the regions arrayList
	 */
	public void clearRegions()						
	{
		for(int i = 0; i<colorRegions.size();)
			colorRegions.remove(i);
	}
	/**
	...
	public void findRegions(Color targetColor) {
		...
		//add the regions to the list of differently colored regions
		colorRegions.add(regions);
	}
	...
	public ArrayList<Point> largestRegion() {
		...
		{
			if(colorRegions.get(size).size() != 0) 
			{
				largest = regions.get(0);
			}
			if(colorRegions.get(size).size()>1)
			{
				for(int j = 1; j< colorRegions.get(size).size(); j++)
					if(largest.size() < colorRegions.get(size).get(j).size())	//compares sizes between index 0 and each other index
					{
						largest = colorRegions.get(size).get(j);
					}
			}
		}
		size++;
	...
	public void recolorImage() {
		...
		//goes through colorRegions list to find each list of regions (stored in the regions arrayList)
		for(ArrayList<ArrayList<Point>> colorRegion: colorRegions)
			for(ArrayList<Point> region: colorRegion) {
